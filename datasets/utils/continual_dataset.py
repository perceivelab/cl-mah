...
class ContinualDataset:
    ...
    @staticmethod
    @abstractmethod
    def get_backbone(additional_classes:int=0) -> nn.Module:
        """
...
'''
def store_previous_masked_loaders(train_dataset: datasets, test_dataset: datasets,
                    setting: ContinualDataset) -> Tuple[DataLoader, DataLoader]:
    """
    Divides the dataset into tasks.
    :param train_dataset: train dataset
    ...
    if setting.i > 0: # for the first task we want the whole dataset
        ...
        #test_mask
        mask_list = [np.array(test_dataset.targets) != label for label in labels_to_remove]
        test_mask = np.stack(mask_list, axis=1)
        test_mask = np.all(test_mask, axis=1)
